...
> Update: the latest EB-ALFRED dataset also fixes several split instructions so SprayBottle tasks use precise object descriptions instead of ambiguous bottle/cleaner wording. Use the same commit above to reproduce results from before the environment and dataset fixes.
